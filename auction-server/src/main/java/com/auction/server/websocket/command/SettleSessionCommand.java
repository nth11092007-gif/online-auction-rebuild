...
import com.auction.server.websocket.AuctionFeedServer;
import com.auction.server.service.SettlementService;

/** SettleSessionCommand - settles and closes an auction session. */
public class SettleSessionCommand implements Command {
  ...
  public void execute(WebSocket conn, JsonObject jsonData) {
    ...
    boolean success =
        settlementService.settleAuction(sessionId);

    if (success) {
      conn.send(gson.toJson(
          new Message("SETTLE_RESULT",
              "Phiên đấu giá đã được kết thúc")));
      Map<String, Object> closedMsg = new HashMap<>();
      closedMsg.put("sessionId", sessionId);
      closedMsg.put("message",
          "Phiên đấu giá đã kết thúc. "
              + "Cảm ơn bạn đã tham gia!");
      Message broadcast =
          new Message("SESSION_CLOSED", closedMsg);
      String jsonBroadcast = gson.toJson(broadcast);
      if (feedServer != null) {
        feedServer.notifyObservers(
            sessionId, jsonBroadcast);
      }
    } else {
      conn.send(gson.toJson(
          new Message("ERROR",
              "Không thể kết thúc phiên đấu giá")));
    }
  }
}
